...
gamma = atan2(v_inf_minus_vect(1),v_inf_minus_vect(2));
...
x = fsolve(@(x)hyp_system(x,v_inf_plus,v_inf_minus,delta,mi_earth),x_0,options);
...
Stheta_inc= sqrt((e_inc^2 - 1)/e_inc);
theta_inf_inc = atan2(Ctheta_inc,Stheta_inc);
% if theta_inf_inc < 0
%     theta_inf_inc = theta_inf_inc +pi;
% end
...
% 
% end
plot(r_inc_vect(1,4000:end),r_inc_vect(2,4000:end),'k')
hold on
Ctheta_out = -1/e_out;

Stheta_out = sqrt((e_out^2 - 1)/e_out);
theta_inf_out = atan2(Ctheta_out,Stheta_out);
% if theta_inf_out < 0
%     theta_inf_out = theta_inf_out +pi;
% end
...
% 
% end
plot(r_out_vect(1,1:5000),r_out_vect(2,1:5000),'y')
