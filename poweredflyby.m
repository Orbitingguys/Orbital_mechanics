function [orbital_parameters_hyp1, orbital_parametershyp2, delta_v,deltaPA]  =  poweredflyby (V_i, V_f,orbital_parameters_planet,mi_planet)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% hyp_orbit.m %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                  
%                                                                                                                %     
% INPUTS:   orbital_parameters_planet[6]    - .a, major semiaxis of planet @sun [km]                             %     
...
[~,V_p]  =  kep2geo (orbital_parameters_planet,mi_sun);
vinf_i  =  V_i-V_p;
vinf_1 = norm(vinf_i);
...
X = fsolve(@(x)hyp_system(x,vinf_2,vinf_1,delta,mi_planet),x_0,options);
ecc1 = X(1);
ecc2 = X(2);
% delta1 = X(3);
% delta2 = X(4);
...
if vinf_i'*jn >= 0
    alfa1 = acos((vinf_i'*ln)/vinf_1);
else
    alfa1 = 2*pi-acos((vinf_i'*ln)/vinf_1);
end
if vinf_f'*jn >= 0
    alfa2 = acos((vinf_f'*ln)/vinf_2);
else
    alfa2 = 2*pi-acos((vinf_f'*ln)/vinf_2);
end
...
if abs(PA2-PA1) > 1e-3
    deltaPA = PA2-PA1;
%     error('cazzo è successo qualcosa che non va')
end
vp_1 = sqrt(vinf_1^2+2*mi_planet/rp);
vp_2 = sqrt(vinf_2^2+2*mi_planet/rp);
delta_v = vp_2-vp_1;

orbital_parameters_hyp1.a = a1;
orbital_parameters_hyp1.ecc = ecc1;
orbital_parameters_hyp1.RAAN = RAAN;
orbital_parameters_hyp1.INCLI = INCLI;
orbital_parameters_hyp1.PA = PA1;
orbital_parametershyp2.a = a2;
orbital_parametershyp2.ecc = ecc2;
orbital_parametershyp2.RAAN = RAAN;
orbital_parametershyp2.INCLI = INCLI;
orbital_parametershyp2.PA = PA2;
