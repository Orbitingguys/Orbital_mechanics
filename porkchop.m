function [Dv,ERROR,TPAR,TOF]=porkchop(t_d,t_a,mi,orbital_parameters_1,orbital_parameters_2)
...
for i=1:m
    for j=1:n
        ...
        if TOF(i,j)>0
            ...
            if TOF(i,j)>TPAR(i,j)
                Dv(i,j)=norm(V2(j,:)-Va)+norm(Vd-V1(i,:));
            end
        end 
    end
end
 DT=TOF-TPAR;
figure(1);surface(t_d,t_a,Dv');
figure(2);contour(t_d,t_a,Dv',50);
figure(3);surface(t_d,t_a,DT');
